        ...
        <section id="view-profile" class="hidden-view">
            <div class="glass-panel rounded-3xl p-6 md:p-10 max-w-md mx-auto shadow-xl">
                ...
                <div id="auth-form-container">
                    ...
                    <div id="auth-error-msg" class="hidden bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4 text-sm font-bold" role="alert">
                        <span class="block sm:inline" id="auth-error-text">Ошибка подключения. Убедитесь, что RLS отключен.</span>
                    </div>
                ...
                <div id="user-dashboard" class="hidden">
                    ...
                    <div id="admin-panel" class="hidden bg-red-50 border-2 border-red-200 rounded-xl p-4 mb-6">
                        ...
                        <div id="admin-error-msg" class="hidden text-red-600 text-xs font-bold mb-2">Ошибка! Возможно таблица 'news' не создана.</div>
    ...
    <script>
        ...
        function switchView(viewId) {
            try {
                ...
                // Обновляем данные, если открыли Топ
                if (viewId === 'leaderboard') {
                    fetchLeaderboard();
                }
            } catch(e) {
                console.error("Ошибка переключения вкладок", e);
            }
        }
        ...
        async function fetchNewsFromDB() {
            try {
                ...
                console.warn("Таблиця news ще не створена, використовуються дефолтні новини.");
            }
        }
            ...
            try {
                const { data, error } = await supabaseClient
                    ...
                    .insert([{ 
                        title: title, 
                        text: text,
                        img: "https://images.unsplash.com/photo-1596733430284-f743727520d6?auto=format&fit=crop&w=400&q=80" 
                    }]);
